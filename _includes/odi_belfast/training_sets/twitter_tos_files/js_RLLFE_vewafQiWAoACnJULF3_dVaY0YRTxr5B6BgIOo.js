...
$("> tbody > tr:visible, > tr:visible",$(t.selector)).removeClass("odd even").filter(":even").addClass("odd").end().filter(":odd").addClass("even")},/**
   ...
   * stylesheets.
   */
add_css:function(e,t){// Add the styles in the normal way.
$("head").prepend(t.data);// Add imports in the styles using the addImport method if available.
var n,r=/^@import url\("(.*)"\);$/gim;if(document.styleSheets[0].addImport&&r.test(t.data))for(r.lastIndex=0;n=r.exec(t.data);)document.styleSheets[0].addImport(n[1])},/**
